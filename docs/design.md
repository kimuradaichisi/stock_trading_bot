...
- `src/main.py`: アプリケーションのエントリーポイント。各モジュールを連携させ、全体のワークフローを制御します。
...
#### `src/config.py`
- **`Config` クラス**:
    - `load_config(config_file: str = 'config.ini') -> dict`: 設定ファイルを読み込み、辞書形式で返します。
    - `get_setting(key: str, default=None)`: 指定されたキーの設定値を取得します。